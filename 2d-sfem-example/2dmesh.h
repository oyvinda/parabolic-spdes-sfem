...
#ifdef transformed_icosasphere_mesh
...
void reorder_vertices()
{
	...
	{
		// Change z coordinate
		double z = vertex_value[i](0,2);
		double val = 1. - 0.5 * std::pow(std::cos(PI * z), 2); //1. / (1. + std::exp(-std::sin(PI * z - PI / 2.) - 1.));
		vertex_value[i](0,0) *= val;
		vertex_value[i](0,1) *= val;

		//vertex_value[i](0,2) = 1. / (1. + std::exp(-std::sin(PI * vertex_value[i](0,2))) );
	}
}
