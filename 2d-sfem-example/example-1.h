...
struct vvt_term
{
	...
	double operator()(const Matrix<double,1,3>& x, const Matrix<int,3,1>& T, int i, int j) const
	{
		...
		double right = 5.*std::pow(std::cos(PI * (x3 - 0.) / 2.), 2), //* (1. + 5.e-1*std::pow(std::sin(2.*PI * (x1*x2 - 0.) / 2.), 2)),
			down = 0.,
			alpha = 1. + 0.*std::pow(std::cos(PI * (x3 - 0.) / 2.), 2);

		double calAconst = 1.;

		vvt.block(0,0,1,3) << calAconst+right*x2*x2, -right*x1*x2, 0.;
		vvt.block(1,0,1,3) << -right*x1*x2, calAconst+right*x1*x1, 0.;
		vvt.block(2,0,1,3) << 0., 0., calAconst + down ;

		return Dphii(x,T,i).dot(vvt*Dphii(x,T,j).transpose()) + alpha * phii(x,T,j) * phii(x,T,i);
...
struct adv_term
{
	...
	double operator()(const Matrix<double, 1,3>& x, const Matrix<int,3,1>& T, int i, int j) const
	{
		...
		double down = 0.5,
			right = 0.*std::pow(std::cos(PI * x3 / 2.), 2);
		
		adv << right*x2, -right*x1, -down*x3;
